...
@section('title')
    <h1 class="title_master_form">{{__('model.tinh_thanh')}}</h1>
@endsection
        ...
        <div class="box-header">            
            <div class="row">
                <div class="col-xs-12 pull-right">
                    <a href="#" data-toggle="modal" data-target="{{'#modal-add-tinhthanh'}}" class="btn btn-flat bg-olive">
                        <i class="fa fa-plus"> Thêm mới</i>
                    </a>
                    @include('danhmuc.tinhthanh.add')
                </div>
            </div>
        </div>
        <div class="box-body">
            <div class="dataTables_wrapper form-inline dt-bootstrap">
                <div class="row">
                    <div class="col-sm-6">
                        @component('components.perpage',['options' => [10,20,50,100], 'default'=> $data->perPage(),'perPage' => $perPage, 'data' => $data, 'routerName' => 'danhmuc.tinhthanh'])
                        @endComponent
                    </div>
                    <div class="col-sm-6">
                        @include('danhmuc.tinhthanh.box-search')
                    </div>
                </div>
                <div class="row">
                    <div class="col-sm-12">
                        <table id="menus" class="table table-bordered table-striped dataTable" role="grid">
                            ...
                            @foreach ($data as $tinhthanh)
                                <tr role="row" class="odd">
                                    <td>{{$tinhthanh->ma}}</td>
                                    <td>{{$tinhthanh->ten}}</td>
                                    <td>{{$tinhthanh->mo_ta}}</td>
                                    <td style="text-align: center;">
                                        @if ($tinhthanh->trang_thai)
                                    ...
                                    <td align="center">
                                        <a href="#" data-toggle="modal" data-target="{{ '#modal-update-tinhthanh-' . $tinhthanh->id }}">
                                            <i class="fa fa-edit"></i>
                                        </a>
                                        @include('danhmuc.tinhthanh.detail')
                                    </td>
                                    <td align="center">
                                        <a href="#" data-toggle="modal" data-target="{{ '#modal-delete-tinhthanh-' . $tinhthanh->id }}">
                                            <i class="fa fa-trash-o"></i>
                                        </a>
                                        @include('danhmuc.tinhthanh.delete')
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
