...
@section('content')
        <div class="box-header">
            <h3 class="title_master_form">Trả lương lần 1</h3>
            <div class="row">
                <div class="col-xs-12">
                    <div style="float: left;">
                        <form action="{{ route('luong.chamcong.capnhatluonglan1',$ten_bang)}}" method="post" >
                            {{ csrf_field() }}
                            <button type="submit" {{($trang_thai_thanh_toan)?'disabled':null}}  class="btn bg-olive btn-file btn-flat" >
                                <i class="fa fa-refresh"> Cập nhật lương lần 1</i>
                            </button>
                        </form>
                    </div>
                    <div style="float: left; margin-left: 10px">
                        <a href="#" data-toggle="modal"  data-target="{{'#modal-add-thanhtoan1'}}" class="btn btn-flat bg-olive">
                            <i class="fa fa-list"> Thanh toán</i>
                        </a>
                        @if($trang_thai_thanh_toan)
                        @else
                         @include('luong.chamcong.traluong.thanhtoan1')
                        @endif
                    </div>
                    ...
                    <div class="col-sm-12">
                        <table id="menus" class="table table-bordered table-striped dataTable" role="grid">
                            ...
                            <tr>
                                ...
                                <th style="width: 20%;">Lương lần 1</th>
                            ...
                            @foreach ($data as $item)
                                <tr role="row" class="odd">
                                    <td>{{$item->ho_ten}}</td>
                                    <td>{{number_format($item->tra_luong_lan_1)}}</td>
                                    <td align="center">
                                        <a href="#" data-toggle="modal" data-target="{{ '#modal-update-luonglan1-' . $item->id }}">
                                            <i class="fa fa-edit"></i>
                                        </a>
                                        @include('luong.chamcong.traluong.edit1')
                                    </td>
